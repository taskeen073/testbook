...
<select name="selectstatus">
	<option value="" <?php if (!isset($_GET['report'])) {echo 'selected';} ?> >ทั้งหมด</option>
    <option value="0" <?php if (isset($_GET['report']) AND $_GET['report'] == '0') {echo 'selected';} ?> >รออนุมัติ</option>
    <option value="1" <?php if (isset($_GET['report']) AND $_GET['report'] == '1') {echo 'selected';} ?> >อนุมัติ</option>
	<option value="2" <?php if (isset($_GET['report']) AND $_GET['report'] == '2') {echo 'selected';} ?> >ไม่อนุมัติ</option>
    <option value="3" <?php if (isset($_GET['report']) AND $_GET['report'] == '3') {echo 'selected';} ?> >ยกเลิก</option>
...
?>
  <tr>
	...
	<a class="green" href="index.php?page=report&action=edit&id=<?php echo $rs['id']; ?>&report=<?php echo $_GET['report']; ?>" title="แก้ไข">
		<i class="ace-icon fa fa-pencil bigger-130"></i>
	</a>
	<a class="blue" href="javascript:popup('report/print.php?id=<?php echo $rs['id']; ?>','',680,500)" title="พิมพ์">
		<i class="ace-icon glyphicon glyphicon-print bigger-130"></i>
	</a>
	<a class="red" href="report/action.php?action=delete&id=<?php echo $rs['id']; ?>&report=<?php echo $_GET['report']; ?>" OnClick="return chkdel();" title="ลบ">
...
<div class="hidden-md hidden-lg">
	<div class="inline pos-rel">
		...
		<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
			<li>
				<a href="index.php?page=report&action=edit&id=<?php echo $rs['id']; ?>&report=<?php echo $_GET['report']; ?>" class="tooltip-success" data-rel="tooltip" title="แก้ไข">
					<span class="green">
			...
			<li>
				<a href="javascript:popup('report/print.php?id=<?php echo $rs['id']; ?>','',680,500)" class="tooltip-info" data-rel="tooltip" title="พิมพ์">
					<span class="blue">
			...
			<li>
				<a href="report/action.php?action=delete&id=<?php echo $rs['id']; ?>&report=<?php echo $_GET['report']; ?>" class="tooltip-error" data-rel="tooltip" title="ลบ" OnClick="return chkdel();">
					<span class="red">
...
<?php if (isset($_GET['action']) AND $_GET['action']=='edit') { 
    ...
    ?>               
<form enctype="multipart/form-data" class="form-horizontal" role="form" name="formregister" method="post" action="report\action.php?action=edit&report=<?php echo $_GET['report'];?>">
...
<div class="form-group">
	<label class="col-sm-3 control-label no-padding-right" >ผู้จอง</label>
      <div class="col-sm-9">
	<input type="text" name="membershow" placeholder="" class="col-xs-10 col-sm-5" value="<?php echo $meResult2['member'];?>" disabled />
...
<div class="form-group">
	<label class="col-sm-3 control-label no-padding-right" for="status" required> สถานะ </label>
      <div class="col-sm-9">
